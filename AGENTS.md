...
| Find and Replace | `src/replace.js`, Replace row in the Swift find bar | Finding stays with `WKWebView.find` (highlight, scroll, select); replacing is document edits. Replace follows TextEdit: replace the selection only if it matches, then the host finds the next. WebKit's find sets the selection *after* its completion handler runs, so nothing may read the selection in that callback — a person pressing Replace is always later. Code blocks are CodeMirror editors whose selection ProseMirror never sees, so a match there is replaced through CodeMirror. Replace All is one transaction (one ⌘Z); inline atoms stand in the text as one placeholder char, their size in the document, so text offsets are document offsets. Undo note: a real ⌘Z reaches ProseMirror's history (WebKit forwards key equivalents to the page first), but Edit ▸ Undo *clicked* sends `undo:` to WebKit's own stack, which never saw a dispatched transaction — pre-existing, affects every non-typed edit. |
| Heading folding | `src/fold.js`, CSS in `app/web/index.html`, View menu in `swift/main.swift` | Folds are plugin state plus node decorations (`mw-fold-heading`, `mw-fold-closed`, `mw-folded`) — never a document change, so the markdown can't be affected. Only top-level headings fold; a section runs to the next heading of the same or higher level. The chevron is the heading's `::after` (`::before` is Crepe's empty-heading placeholder), placed at -18px so it clears Crepe's block handle, which ends 16px left of the text; clicks on it arrive as a mousedown on the heading left of its box. A selection landing in hidden content unfolds it; `MW.revealFind` is called by Swift before every native find, because WebKit's find skips `display: none` text; the outline unfolds before scrolling. **Mapping gotcha**: Milkdown rewrites a heading's markup (its `id`) on every text edit, which makes `mapResult(pos, 1)` report the heading as deleted — `mapHeading` falls back to the other side. PDF export shows everything (`html.mw-pdf-export` exempts `.mw-folded`). |
| Keeping your place across ⌘/ | `src/sourcesync.js` | The top line of one view becomes the top line of the other, and the caret rides along separately — anchoring on the caret alone strands you whenever you have scrolled away from it, which is most of the time. There is no position map between the views (the markdown is re-serialized whole; remark returns no source offsets), so the shared unit is the sequence of blocks. **To the source**: serializing everything before the anchor's block estimates its offset, and searching for the block's own first line near that estimate snaps it onto the true one — the estimate drifts because the join handlers suppress separators. The run of text just before the anchor then refines it within the block. **Back**: parsing the source up to the offset leaves that block as the last child at every level, so the path down the tail of the parse is the path to the block, nesting included. **Scrolling a textarea to a character** has no API and lines wrap, so `lineTable` lays the same text out again in a hidden element of the same width and font, one span per line, and reads back each `offsetTop` — which is the `scrollTop` that puts that line at the top. Two ordering traps, both measured: the rendered view must be asked what is at its top *before* `source-open` hides it, and focusing a textarea reveals its selection asynchronously, overwriting a `scrollTop` set in the same task — so the scroll goes first and the selection lands on a line already on screen, giving the reveal nothing to do. |
